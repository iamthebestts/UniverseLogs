...
export class MemoryCache<T> {
   ...
   */
  async getOrSet(key: string, factory: () => T | Promise<T>, options: SetOptions = {}): Promise<T> {
    ...
    const promise = (async () => {
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error("Cache factory timeout")), 30000),
        );

        const value = await Promise.race([factoryPromise, timeoutPromise]);
        this.set(key, value, options);
        return value;
      } finally {
  ...
  stats(): {
    ...
    const hitRate = totalRequests > 0 ? this.stats_data.hits / totalRequests : 0;
